...
pub fn logFn(
    ...
    var buf = std.ArrayList(u8).initCapacity(allocator, prefix.len + format.len + 1) catch return;
    defer buf.deinit(allocator);
    buf.writer(allocator).print(prefix, .{}) catch return;
    buf.writer(allocator).print(format, args) catch return;
...
fn sendElogWithCause(src: SourceLocation, comptime level: c_int, cause: anyerror, comptime fmt: []const u8, args: anytype) void {
    ...
    var memctx = mem.getErrorContextThrowOOM();
    const elog_allocator = memctx.allocator();
    var buf = std.ArrayList(u8).initCapacity(elog_allocator, fmt.len + err_name.len + 20) catch unreachable;
    buf.writer(elog_allocator).print(fmt, args) catch unreachable;
    buf.writer(elog_allocator).writeAll(": ") catch unreachable;
    buf.writer(elog_allocator).writeAll(err_name) catch unreachable;
    buf.writer(elog_allocator).writeByte(0) catch unreachable;
